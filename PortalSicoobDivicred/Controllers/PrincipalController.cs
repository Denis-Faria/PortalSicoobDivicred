        ...
        public ActionResult BuscaHistoricoCheque(string contaCorrente)
        {
            ...
            if (logado)
            {
                ...
                foreach (var cheque in cheques)
                {
                    ...
                    TempData["DataDevolucao" + count] =
                        Convert.ToDateTime(cheque["datadevolucao"]).ToString("dd/MM/yyyy");
                ...
                foreach (var bloqueio in bloqueios)
                {
                    ...
                    TempData["DataBloqueio" + count2] =
                        Convert.ToDateTime(bloqueio["databloqueio"]).ToString("dd/MM/yyyy");
                    ;
                    TempData["DataDesbloqueio" + count2] =
                        Convert.ToDateTime(bloqueio["datadesbloqueio"]).ToString("dd/MM/yyyy");
                    ;
                    count2++;
                }

                return PartialView("ResultadoHistoricoTalao");
            }

            return RedirectToAction("Login", "Login");
        }

        public ActionResult SolicitaDesbloqueio(string contaCorrente)
        {
            var verificaDados = new QueryMysqlWebdesk();
            var verificaDadosTesouraria = new QueryMysqlTesouraria();
            var logado = verificaDados.UsuarioLogado();
            if (logado)
            {
                var cookie = Request.Cookies.Get("CookieFarm");
                if (cookie != null)
                {
                    ...
                    var cheques = verificaDadosTesouraria.RetornaHistoricoCheques( contaCorrente );

                    verificaDados.CadastraSolicitacao("53", "192", "132",
                        "Favor liberar os talões para o cooperado:" + cheques[0]["nome"] +
                        " com a conta corrente de n°:" + contaCorrente, dadosUsuario[0]["id"], "", "N");
                    ...
                    return RedirectToAction("Principal", "Principal",
                        ...
                        {
                            Acao = "Dashboard",
                            Mensagem = "Solicitação de desbloqueio realizada com sucesso!",
                            Controlle = "Principal"
                        });
                }
            }
